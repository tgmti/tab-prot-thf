...
/*/{Protheus.doc}aFind
  ==============================================================================
    ...
    @obs
        ...
        Se o Tipo de Callback for diferente de bloco, será montado como {|x| x[nInd] == bCallback }
...
/*/
//============================================================================\
User Function aFind( aOrigin, xFind, uDefault, nInd, nIni, nEnd )
Return aFind(aOrigin, xFind, uDefault, nInd, nIni, nEnd)

Static Function aFind(aOrigin, xFind, uDefault, nInd, nIni, nEnd)
    Local nPos, bCallback
	Local uRet:= uDefault
    Default nInd:= 1
    ...
    If ValType(xFind) == 'B'
        bCallback:= xFind
    Else
        bCallback:= &('{|x| x[nInd] == xFind }')
    EndIf

    nPos:= aScan( aOrigin, bCallback, nIni, nEnd )
	If nPos > 0
		uRet:= aOrigin[nPos]
	EndIf
