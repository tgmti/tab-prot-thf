...
module.exports = {
    tables(request, response) { return query('SX2', TablesSchema, request, response); },
    fields(request, response) { return query('SX3', FieldsSchema, request, response); },
    params(request, response) { return query('SX6', ParamsSchema, request, response); },
    indexes(request, response) { return query('SIX', IndexesSchema, request, response); },
}


async function query(table, fieldsDefault, request, response) {

    if (process.env.DEBUG) console.log('request', request.path);
    ...
    try {
        ...
        const filter = filterFields(configdb.sgdb, request.query.filter || request.query.search, fieldsNormalized);
...
function mountQuery(configdb, table, select, where, order, page, pageSize) {
    ...
    if (process.env.DEBUG) console.info(query, values);
