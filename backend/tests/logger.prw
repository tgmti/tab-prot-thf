...
Method New( cContext, cOutDir ) Class Logger
	::Context := cContext
	Return ::Start(cOutDir)

Method Start(cOutDir) Class Logger
	Local cSep := If( isSrvUnix(), '/', '\' )
	Local cOutFile

	Default cOutDir := CurDir() + cSep + 'logs'

	cOutFile := cOutDir + cSep + ::Context + '.log'
